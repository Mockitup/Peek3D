...
const THREE_JS: &str = include_str!("frontend/three-bundle.min.js");
const VIEWER_JS: &str = include_str!("frontend/viewer3d.js");
...
fn main() {
    ...
    let _webview = WebViewBuilder::new()
        .with_custom_protocol("peek3d".to_string(), move |_id, request| {
            ...
            if uri == "/" || uri == "/index.html" {
                ...
                let st = state_proto.lock().unwrap();
                match &st.model_bytes {
                    Some(bytes) => {
                        let data = Arc::clone(bytes);
                        let model_type = st.model_type.clone().unwrap_or_default();
                        drop(st);
                        let content_type = match model_type.as_str() {
                    ...
                    None => wry::http::Response::builder()
                        .status(404)
                        .body(Cow::Borrowed(b"No model" as &[u8]))
                        .unwrap(),
                }
            } else {
                wry::http::Response::builder()
        ...
        .with_url("http://peek3d.localhost/")
...
fn build_html() -> String {
    let scripts = format!(
        "<script>{}</script>\n<script>{}</script>\n<script>{}</script>",
        escape_for_script_tag(THREE_JS),
